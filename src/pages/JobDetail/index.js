...
import { creatCv } from "../../services/cvService";
...
function JobDetail() {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      const result = await getDetailJob(params.id);
  ...
  const onFinish = async (values) => {
    ...
    const result = await creatCv(values);
    if (result){
      form.resetFields();
      noti.success({
        message: "Gửi yêu cầu thành công!",
        description: "Nhà tuyển dụng sẽ liên hệ với bạn trong thời gian lớn nhất.",
      });
    } else {
      noti.error({
